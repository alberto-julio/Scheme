...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        w = scheme_eval(expressions.rest.first ,env)
        env.define(signature,w)
        return signature
        # END PROBLEM 4
...
def do_or_form(expressions, env):
    ...
    while expressions.rest != nil:
        or_expr = scheme_eval(expressions.first, env)
        if is_scheme_true(or_expr):
            return or_expr
        else:
            expressions = expressions.rest
    return scheme_eval(expressions.first,env,True)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        binded = bindings.first
        validate_form(bindings.first, 2,2)
        oogabooga = binded.first
        chongawonga = scheme_eval(binded.rest.first,env)
        names = Pair(oogabooga,names)
        vals = Pair(chongawonga, vals)
